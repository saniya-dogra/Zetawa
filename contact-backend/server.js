...
// --- CORS & logger (replace previous cors block with this) ---
const allowedOrigins = [
  'https://zetawa.com',
  'https://www.zetawa.com',           // IMPORTANT: include www variant
  'http://localhost:5173',
  'https://zetawa-dark.vercel.app',
  'https://zetawa.vercel.app'
];

// Small request logger to see the origin in server logs (helpful for debugging)
app.use((req, res, next) => {
  console.log(`[${new Date().toISOString()}] ${req.method} ${req.originalUrl} - Origin: ${req.headers.origin || 'no-origin'}`);
  next();
});

// Strict CORS that only allows the origins above. Accepts preflight requests.
app.use(cors({
